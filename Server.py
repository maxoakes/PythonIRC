...
class Server:    
    ...
    def listenForConnections(self):
        ...
        self.createRoom(Codes.ROOM_DEFAULT_NAME,self.serverName,"The default room",False)
    ...
    def listenToClient(self, address, port, clientsocket):
        ...
        if (not username):
            ...
            print("%s Username returned as blank" % Codes.STR_ERR)
            return
        ...
        print("%s Now listening for messages for %s" % (Codes.STR_INFO, username))        
    ...
    def unregisterUser(self, user):
        try:
            ...
            print("%s %s was not found in active user list" % (Codes.STR_ERR, user))
        try:
    ...
    def handleUserCommand(self, message, clientsocket):
        ...
        if message.messageType == Codes.MSG_ROOM:
            ...
            if message.content == Codes.ROOM_LIST:
                ...
                self.sendMessage(
                    Message(
                        self.serverName, Codes.MSG_ROOM, (Codes.ROOM_LIST, roomListString)),
                        clientsocket)
                return
            ...
            if isinstance(message.content, tuple):
                ...
                if message.content[0] == "join":
                    if message.content[1] in self.rooms.keys():
                        ...
                        if message.sender not in self.rooms[message.content[1]].currentUsers:
                            ...
                            print("%s User tried to join room they were already in" % Codes.STR_INFO)
                            self.sendCommandFail(Codes.MSG_ROOM, Codes.ROOM_JOIN, clientsocket)
                    else:
                        self.sendCommandFail(Codes.MSG_ROOM, Codes.ROOM_JOIN, clientsocket)
                        ...
                        if message.sender in self.rooms[message.content[1]].currentUsers:
                            try:
                                ...
                                self.sendMessage(Message(
                                    self.serverName, Codes.MSG_ROOM, (Codes.ROOM_LEAVE, message.content[1])),
                                    clientsocket)
                            except:
                                print("%s Failed to remove user from room" % Codes.STR_ERR)
                                traceback.print_exc()
                                self.sendCommandFail(Codes.MSG_ROOM, Codes.ROOM_LEAVE, clientsocket)
                        else:
                ...
                if message.content[0] == "create":
                    ...
                    if message.content[1] not in self.rooms.keys():
                        ...
                        self.sendMessage(Message(
                            self.serverName, Codes.MSG_ROOM, (Codes.ROOM_CREATE, message.content[1])),
                            clientsocket)
                    else:
                        self.sendCommandFail(Codes.MSG_ROOM, Codes.ROOM_CREATE, clientsocket)
    ...
    def sendCommandFail(self, messageType, subtype, clientsocket):
        self.sendMessage(Message(self.serverName, messageType, (subtype, Codes.ACT_FAIL)),
            clientsocket)
